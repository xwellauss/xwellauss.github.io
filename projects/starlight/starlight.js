...
var PThread = {
 ...
 allocateUnusedWorker() {
  var worker;
  var pthreadMainJs = locateFile("starlight.worker.js");
...
function ___syscall_chdir(path) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(2, 1, path);
 try {
...
function ___syscall_poll(fds, nfds, timeout) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(7, 1, fds, nfds, timeout);
 try {
  var nonzero = 0;
...
function setCanvasElementSizeMainThread(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(9, 1, target, width, height);
...
function _emscripten_request_fullscreen_strategy(target, deferUntilInEventHandler, fullscreenStrategy) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(8, 1, target, deferUntilInEventHandler, fullscreenStrategy);
 var strategy = {
...
function _emscripten_set_wheel_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(11, 1, target, userData, useCapture, callbackfunc, targetThread);
 target = findEventTarget(target);
 if (!target) return -4;
...
function _emscripten_websocket_send_binary(socketId, binaryData, dataLength) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(12, 1, socketId, binaryData, dataLength);
...
var _environ_get = function(__environ, environ_buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 1, __environ, environ_buf);
 var bufSize = 0;
...
var _environ_sizes_get = function(penviron_count, penviron_buf_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(14, 1, penviron_count, penviron_buf_size);
...
function _fd_close(fd) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(15, 1, fd);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  FS.close(stream);
  return 0;
...
function _fd_read(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(16, 1, fd, iov, iovcnt, pnum);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(17, 1, fd, offset_low, offset_high, whence, newOffset);
...
function _emscripten_set_window_title(title) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 1, title);
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, ___syscall_chdir, ___syscall_faccessat, ___syscall_fcntl64, ___syscall_ioctl, ___syscall_openat, ___syscall_poll, _emscripten_request_fullscreen_strategy, setCanvasElementSizeMainThread, getCanvasSizeMainThread, _emscripten_set_wheel_callback_on_thread, _emscripten_websocket_send_binary, _environ_get, _environ_sizes_get, _fd_close, _fd_read, _fd_seek, _fd_write, _emscripten_set_window_title, _emscripten_get_device_pixel_ratio ];
...
var wasmImports = {
 /** @export */ ImGui_ImplGlfw_EmscriptenOpenURL: ImGui_ImplGlfw_EmscriptenOpenURL,
...
var ___emscripten_embedded_file_data = Module["___emscripten_embedded_file_data"] = 130559624;

var ___start_em_js = Module["___start_em_js"] = 130579024;

var ___stop_em_js = Module["___stop_em_js"] = 130579124;
